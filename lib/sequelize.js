...
// Define Admin model (matches existing admins table schema)
...
const initializeDatabase = async (retries = 3) => {
  try {
    ...
    if (process.env.NODE_ENV === "development" || process.env.VERCEL !== "1") {
      ...
      // Use force: true for SQLite, alter: true for PostgreSQL
      const isSQLite = process.env.DATABASE_URL && process.env.DATABASE_URL.startsWith("sqlite:");
      const syncOptions = isSQLite ? { force: true } : { alter: false };
    ...
    // In development, if Postgres fails, fallback to SQLite automatically
    ...
    if (
      ...
      (error.name === "SequelizeConnectionError" ||
        ...
        /terminated unexpectedly/i.test(String(error)))
    ) {
      try {
        console.warn(
          "PostgreSQL connection failed in development. Falling back to SQLite (./database.sqlite)."
        );
        process.env.DATABASE_URL = "sqlite:./database.sqlite";

        // Recreate sequelize instance for SQLite
        sequelize = new Sequelize(process.env.DATABASE_URL, {
          dialect: "sqlite",
          storage: process.env.DATABASE_URL.replace("sqlite:", ""),
          logging: process.env.NODE_ENV === "development" ? console.log : false,
